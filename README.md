...
这次更新修复新建小镇时全部居民已经分配模型后无法继续改绑的问题，单人或批量改选模型都会明确进入改绑操作；同时让 MiniMax 官方接入支持 MiniMax M3 图片输入，并让 M3 的居民决策默认关闭思考，优先快速返回结果；集中改善小镇头顶对话、聊天消息、公开想法、居民动作文字和操作提示的断行与分页；修复人物进入建筑、树木等前景区域时局部画面突然变暗的问题；并建立 Android 横屏、安全区域、触控滚动与系统输入法的适配基础。
